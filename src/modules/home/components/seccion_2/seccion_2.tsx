...
const Seccion_2 = () => {
  ...
  // ✅ NUEVO: id del producto a vender; null = no está en modo venta
  const [idVendiendo, setIdVendiendo] = useState<number | null>(null);
  ...
  // ✅ NUEVO: abre el componente Vender con el id del producto
  const handleVenderProducto = (id: number) => {
    setIdVendiendo(id);
  };
  ...
  // ✅ Si hay un producto seleccionado para vender, renderiza Vender en su lugar
  if (idVendiendo !== null) {
    return (
      <Vender
        prdcid={idVendiendo}
        onVolver={() => setIdVendiendo(null)}
      />
    );
  }
      ...
      <div className={styles.toolbar}>
        <div className={styles.tabs}>
          <button
            ...
            onClick={() => { setVista("producto"); setPagina(1); }}
          >
            Producto
          </button>
          <button
            className={`${styles.tab} ${vista === "categoria" ? styles.active : ""}`}
            onClick={() => { setVista("categoria"); setPagina(1); }}
          >
            Categoría
          </button>
          <button
            className={`${styles.tab} ${vista === "marca" ? styles.active : ""}`}
            onClick={() => { setVista("marca"); setPagina(1); }}
          >
            Marca
          </button>
        </div>
                ...
                <tr key={p.prdcid}>
                  ...
                  <td>
                    <div className={styles.acciones}>
                      {/* ✅ Botón Vender — llama handleVenderProducto con el id */}
                      <button
                        className={styles.btnVender}
                        onClick={() => handleVenderProducto(p.prdcid)}
                      >
                        Vender
                      </button>
                      <button
